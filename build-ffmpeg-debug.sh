...
do
	if [ "${ARCH}" == "i386" ]; then
        ...
        EXTRA_CONFIG="--arch=i386 --target-os=darwin --enable-cross-compile --cpu=i386"
        EXTRA_CFLAGS="-arch i386 -fPIE -miphoneos-version-min=6.0 -O0 -fno-inline -g"
        EXTRA_LDFLAGS="-fPIE -miphoneos-version-min=6.0"
    else
        PLATFORM="iPhoneOS"
        EXTRA_CONFIG="--arch=arm --target-os=darwin --enable-cross-compile --disable-armv5te"
        EXTRA_CFLAGS="-w -arch ${ARCH} -fPIE -miphoneos-version-min=6.0 -O0 -fno-inline -g"
        EXTRA_LDFLAGS="-fPIE -miphoneos-version-min=6.0"
	fi
    ...
    if [ ! -d "$OUTPUT_DIR" ]; then
    	...
    	./configure --disable-shared --enable-static --enable-pic --disable-programs \
        --enable-debug=3 --disable-optimizations --disable-stripping --disable-asm --assert-level=2 \
